...
class NotificationSyncherException(Exception):
    pass


class NotificationSyncher:
    """Handles synching Notifications from a Google spreadsheet."""
    ...
    def __init__(self, sheet_id: str = None) -> None:
        ...
        if not self.sheet_id:
            raise NotificationSyncherException("Sheet ID not set.")

        self._sheet_data: Optional[SheetData] = None

    def fetch_data(self) -> None:
        self._sheet_data = {}
        csv_data = self._fetch_csv_data()
        self._sheet_data = self._get_sheet_data_from_csv_file(io.StringIO(csv_data))

    @transaction.atomic()
    def create_non_existing_and_update_existing_notifications(self) -> Tuple[int, int]:
        num_of_created = self.create_non_existing_notifications()
        num_of_updated = self.update_notifications(NotificationTemplate.objects.all())
        return num_of_created, num_of_updated

    @transaction.atomic()
    def create_non_existing_notifications(self) -> int:
    ...
    def sheet_data(self) -> SheetData:
        if self._sheet_data is None:
            self.fetch_data()
        assert self._sheet_data is not None  # Mypy wants me
    ...
    def _fetch_csv_data(self) -> str:
        try:
            ...
            raise NotificationSyncherException(
                f"Error fetching data from sheet: {e}"
            ) from e
    ...
    def _create_or_update_notification_using_sheet_data(
        ...
        for language in self.LANGUAGES:
            ...
            # This update method is called inside a transaction so the notification
            # will not get actually saved in case of an error.
